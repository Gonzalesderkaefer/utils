...
void _fs_walk(ListStats *lstats, const char *directory) {
    Path **subpaths;
    list_from_stats(subpaths, lstats);
    
    // Open directory
    DIR *cur_dir = opendir(directory);
    if (cur_dir == NULL) { // Error checking
        return;
    }
    struct dirent *dircontent; // This is one directory entry
    // Iterate through directory
    while((dircontent = readdir(cur_dir)) != NULL) {
        // Don't add this or parent dir
        if (!strcmp(".", dircontent->d_name) || !strcmp("..", dircontent->d_name)) {
            continue;
        }

        Path *this_elem = path_init(directory);
        if (this_elem == NULL) {
            continue;
        }
        path_append(this_elem, dircontent->d_name);

        // Metadata for element
        struct stat this_stat;
        lstat(path_str(this_elem), &this_stat);

        // Check for access, if no access skip
        if (access(path_str(this_elem), R_OK) == -1) { 
            path_free(this_elem);
            continue;
        }

        // Add element to list
        list_push(subpaths, this_elem);

        if (S_ISDIR(this_stat.st_mode)) {
            _fs_walk(list_stat(subpaths), path_str(this_elem));
        }
    }
    closedir(cur_dir);
}
...
Path **fs_walk(const char *directory) {
    Path **subpaths = NULL;
    list_init(subpaths);
    _fs_walk(list_stat(subpaths), directory);
